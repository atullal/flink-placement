...
public class DeclarativeSlotManager implements SlotManager {
    ...
    public void processResourceRequirements(ResourceRequirements resourceRequirements) {
        ...
        // now try to allocate resources for resource requirements
        checkResourceRequirements();
    }
    ...
    public boolean registerTaskManager(
        ...
        checkInit();
        LOG.info(
                "mylog registerTaskManager Registering task executor {} under {} at the slot manager.",
    ...
    private int internalTryAllocateSlots(
        ...
        //
        // freeSlots.iterator().next().getTaskManagerConnection().getTaskExecutorGateway().getAddress();

        LOG.info(
                "lei: number of resourceRequirements: {} Their resourceProfile: {}",
                freeSlots.size(),
                requiredResource);
        ...
        for (int x = 0; x < resourceRequirement.getNumberOfRequiredSlots(); x++) {
            ...
            LOG.info("lei: number of free slots: {}", freeSlots.size());
    ...
    private void allocateSlot(
        ...
        LOG.info(
                "mylog DeclarativeSlotManager allocateSlot Starting allocation of slot {} for job {} with resource profile {}.",
                slotId,
                jobId,
                resourceProfile);
