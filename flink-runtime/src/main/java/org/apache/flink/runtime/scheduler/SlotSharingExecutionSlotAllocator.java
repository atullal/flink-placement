 ...
 * a physical slot for the shared slot and then allocates llei: This is locationMatchingogical slots
 * from it for scheduled tasks. The physical slot is lazily allocated for a shared slot, upon any
 * hosted subtask asking for the shared slot. Each subsequent sharing subtask allocates a logical
 * slot from the existing shared slot. The shared/physical slot can be released only if all the
 * requested logical slots are released or canceled.
 */
class SlotSharingExecutionSlotAllocator implements ExecutionSlotAllocator {
    ...
    private Map<ExecutionVertexID, String> mapEVID2TMID =
            new HashMap<>(); // mapping from ExecutionVertexID to IP of task manager
    ...
    public List<SlotExecutionVertexAssignment> allocateSlotsFor(
        ...
        for (ExecutionVertexID evid : executionVertexIds) {
            LOG.info("mylog allocateSlotsFor ExecutionVertexID {}", evid);
        }
    ...
    public List<SlotExecutionVertexAssignment> allocateSlotsFor_Custom(
            List<ExecutionVertexID> executionVertexIds,
            Map<ExecutionVertexID, String> paramapEVID2TMID) {
        this.mapEVID2TMID = paramapEVID2TMID;

        for (ExecutionVertexID evid : executionVertexIds) {
            LOG.info("mylog allocateSlotsFor_Custom ExecutionVertexID {}", evid);
        }

        SharedSlotProfileRetriever sharedSlotProfileRetriever =
                sharedSlotProfileRetrieverFactory.createFromBulk(new HashSet<>(executionVertexIds));

        // set of tasks to be deployed after configuring slot sharing
        Map<ExecutionSlotSharingGroup, List<ExecutionVertexID>> executionsByGroup =
                executionVertexIds.stream()
                        .collect(
                                Collectors.groupingBy(
                                        slotSharingStrategy::getExecutionSlotSharingGroup));
        LOG.info(
                "mylog allocateSlotsFor_Custom executionsByGroup_size={} ",
                executionsByGroup.size());
        for (ExecutionSlotSharingGroup essg : executionsByGroup.keySet()) {
            LOG.info(
                    "mylog allocateSlotsFor_Custom executionsByGroup_val_size={} ",
                    executionsByGroup.get(essg).size());
            for (ExecutionVertexID evid : executionsByGroup.get(essg)) {
                LOG.info("mylog allocateSlotsFor_Custom executionsByGroup_val_id={} ", evid);
            }
            for (ExecutionVertexID evid : essg.getExecutionVertexIds()) {
                LOG.info("mylog allocateSlotsFor_Custom executionsByGroup_key_id={} ", evid);
            }
        }

        // for each task, get a future slot (to be assigned)
        Map<ExecutionSlotSharingGroup, SharedSlot> slots =
                executionsByGroup.keySet().stream()
                        .map(group -> getOrAllocateSharedSlot(group, sharedSlotProfileRetriever))
                        .collect(
        ...
        for (Map.Entry<ExecutionSlotSharingGroup, List<ExecutionVertexID>> entry :
            ...
            // only 1 iteration when slot sharing is disabled
            for (ExecutionVertexID executionId : executionIds) {
                LOG.info(
                        "mylog allocateSlotsFor_Custom getPhysicalSlotRequestId {}",
                        slots.get(group).getPhysicalSlotRequestId());

                CompletableFuture<LogicalSlot>
                        logicalSlotFuture = // MyallocateLogicalSlot(executionId);
                ...
                LOG.info(
                        "mylog SharedSlot allocateLogicalSlot executionId {} logicalSlotFuture {}",
                        executionId,
                        logicalSlotFuture.toString());
            }
        }
        ...
        LOG.info(
                "mylog allocateSlotsFor_Custom executionsByGroup_size {}",
                executionsByGroup.size());
        LOG.info("mylog allocateSlotsFor_Custom slots_size {}", slots.size());
        LOG.info("mylog allocateSlotsFor_Custom assignments_size {}", assignments.size());
    ...
    // for each task slot request: call this method (scheduler request slot from slot pool)
    private SharedSlot getOrAllocateSharedSlot(
        ...
        return sharedSlots.computeIfAbsent(
                ...
                group -> {
                    LOG.info(
                            "mylog getOrAllocateSharedSlot group.getExecutionVertexIds -------------------- ");
                    for (ExecutionVertexID evid : group.getExecutionVertexIds()) {
                        LOG.info(
                                "mylog getOrAllocateSharedSlot group.getExecutionVertexIds {}",
                                evid);
                    }
                    ...
                    LOG.info(
                            "lei: physicalSlotResourceProfile {} {}",
                            physicalSlotResourceProfile,
                            physicalSlotResourceProfile.getPreferredLocation());

                    SlotProfile slotProfile =
                            sharedSlotProfileRetriever.getSlotProfile(
                                    group, physicalSlotResourceProfile);

                    // physical slot request with preferred info
