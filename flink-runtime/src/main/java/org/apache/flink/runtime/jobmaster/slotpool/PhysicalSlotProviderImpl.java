...
public class PhysicalSlotProviderImpl implements PhysicalSlotProvider {
    ...
    public CompletableFuture<PhysicalSlotRequest.Result> allocatePhysicalSlot(
        ...
        LOG.info(
                "mylog Received slot request [{}] with resource requirements: {}",
                slotRequestId,
                resourceProfile);

        // local slot pool is empty --> request new slots from resource manager (slot manager)
    ...
    private Optional<PhysicalSlot> tryAllocateFromAvailable(
        ...
        // Note: always return empty here: slotInfoList always empty
