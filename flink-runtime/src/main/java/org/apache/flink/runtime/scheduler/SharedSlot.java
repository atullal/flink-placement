...
class SharedSlot implements SlotOwner, PhysicalSlot.Payload {
    ...
    private CompletableFuture<SingleLogicalSlot> allocateNonExistentLogicalSlot(
        ...
        LOG.info("mylog allocateNonExistentLogicalSlot Request a {}", logMessageBase);

        logicalSlotFuture =
                slotContextFuture.thenApply(
                        physicalSlot -> {
                            LOG.info(
                                    "mylog allocateNonExistentLogicalSlot Allocated {} physicalSlot.getTaskManagerLocation {} ",
                                    logMessageBase,
        ...
        logicalSlotFuture.exceptionally(
                cause -> {
                    LOG.info(
                            "mylog allocateNonExistentLogicalSlot Failed {}",
                            logMessageBase,
                            cause);
    ...
    private SingleLogicalSlot createLogicalSlot(
        ...
        LOG.info(
                "mylog createLogicalSlot {} {} ",
                physicalSlot.getTaskManagerLocation(),
                logicalSlotRequestId.toString());
